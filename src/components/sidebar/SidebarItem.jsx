...
function SidebarItem(props) {
  ...
  return (
    <>
      ...
      <NavLink style={{
        marginTop: "5px"
      }} to={item.path} className="link" activeclassname="active" onClick={item.subNav && showSubNav} title={item.title}>
      ...
      {subNav && item.subNav.map((item, index) => {
        return(
          ...
          <NavLink  to={item.path} key={index} className="nav-subItem link" >
            <div  className="icon">{item.icon}</div>

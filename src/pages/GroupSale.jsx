...
import TableSaleGroup from '../components/sales/TableSaleGroup';
...
function GroupSale(props) {
  ...
  useEffect(() => {
    // console.info(selectedClientId)
  }, [selectedClientId])
  /* Set props */
  const setDataClienteProp = (cliente) => {
    setDataCliente(cliente)
  }

  const setSelectedClienteProp = (cliente) => {
    setSelectedCliente(cliente)
  }

  const setSelectedClienteIdProp = (cliente) => {
  ...
  const handleChange = (e) => {
    e.preventDefault();
    // No se permiten espacios en blanco al final
    const inputValue = e.target.value.trim();
    setNumeroComanda(inputValue);
    setDataPost((prevDataPost) => ({
      ...prevDataPost,
      NumeroComanda: e.target.value,
      // Inicializa la fecha al ingreso de número de comanda
      Fecha: defaultDate || dayjs().format('YYYY-MM-DD'),
    }));
  };

  // Actualizar el cliente seleccionado
  const setDataPostProp = (clienteSeleccionado) => {
    setDataPost((prevDataPost => ({
      ...prevDataPost,
  ...
  const setDataPostPropDate = (fechaSeleccionada) => {
    setDataPost((prevDataPost => ({
      ...prevDataPost,
    ...
    <div className={isOpen ? "wrapper" : "side"}>
      <form className="comanda-form" onSubmit={postSale}>
        <section className="comanda">
          <h2>Comanda digital grupal</h2>
            <div className="container-fluid m-0 p-0">
              <div className="row comanda-flex-controls">
                <div className="col-sm-3 col-md-3 col-lg-3 comanda-control-1">
                  <div className="form-floating mb-3">
                    <input
                      ...
                      required
                    />
                    <label htmlFor="input-comanda"># de Comanda</label>
                  </div>
                </div>
                <div className="col-sm-9 col-md-9 col-lg-6 comanda-control-2">
                  <SearchBarDrop

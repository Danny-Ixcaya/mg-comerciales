...
import SwalCashBox from "../SwalCashBox";
// Caja abierta
...
function ModalAdd(props) {
  ...
  //Hacer la peticion para obtener el saldo de la ultima caja para validar si se permite el registro del gasto
  const fetchUltimaCaja = async () => {
    try {
      ...
      if (response.ok) {
        ...
        console.log("Error fetching ultima caja");
      }
    } catch (error) {
      console.log(error);
    }
  ...
  const onSubmit = async (valores, { resetForm }) => {
    ...
    if(bodyProvider.total > ultimaCaja.caja) {
      Swal.fire({
        position: "center",
        icon: "warning",
        title: "No puede registrar el gasto el total supero la cantidad en caja",

...
function TableSaleGroup({
  ...
  selectedCliente,
  selectedClientId,
  setSelectedClienteProp,
  setSelectedClienteIdProp,
  setDataPostProp,
}) {
  ...
  useEffect(() => {
    // Object.values(props.selectedClienteProp).some(value => value === ""
    setIsFormValid(saleDetail.length > 0 && selectedClientId);
  }, [saleDetail, selectedClientId]);
  ...
  return (
    <div className="comanda-digital">
      <Table responsive>
        ...
        {saleDetail.length === 0 ? (
          <tr>
            <td colSpan="5" align="center">
              Debes seleccionar los items, da click en el buscador 😊 <br />
              {/* También debes seleccionar cliente en el botón observaciones ✅ */}
            </td>
          </tr>
        ) : (
        ...
        {saleDetail.length > 0 ? (
        <tr className="table-total-items">
          <td colSpan={3} className="total-td-title">
            Total:
          </td>
          <td >
            Q.{totalComanda.toFixed(2)}
          </td>
        </tr>
        ) : (
          null
        ...
        <table className="table responsive table-resumen">
          ...
          <tbody className="table-resumen-body">
            <tr className="row-resumen">
              <th className="table-summary-item table-no-comanda">
                No. {noComanda}
              </th>
            </tr>
            <tr>
              <td className="row-resumen table-summary-item">Fecha: 
                <input 
                  type="date" 
                  value={defaultDate}
                  onChange={changeDate}
            ...
            <tr className="row-resumen">
              <td className="row-resumen table-summary-item d-flex justify-content-center">
                <ModalObservaciones 
                  observaciones={observaciones}
                  changeObservaciones={changeObservaciones}
                  cliente={dataCliente}
                  setDataClienteProp={setDataClienteProp}
                  selectedClientId={selectedClientId}
                  selectedCliente={selectedCliente}
                  setSelectedClienteProp={setSelectedClienteProp}
                  setSelectedClienteIdProp={setSelectedClienteIdProp}
                  setDataPostProp={setDataPostProp}
                />
              </td>
            </tr>
            <tr className="row-resumen">
              <td className="resumen-total table-summary-item"><span className="total-comanda"> Total: Q.{totalComanda.toFixed(2)}</span></td>
            </tr>
            <tr className="row-resumen">
